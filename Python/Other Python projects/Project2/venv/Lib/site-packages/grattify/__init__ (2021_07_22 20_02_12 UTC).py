...
import sys, requests, youtube_dl, os, traceback, spotipy, urllib.request, urllib.parse, urllib.error, pprint, re
...
def findNthBestLink(n, artist, title):
    ...
    badKeywords = [bk for bk in badKeywords if searchString.find(bk) < 0]
...
def printResults(ytResults, bestToWorst):
    ...
    print("\n\n\n")
    print("BY ALGORITHM::")
    smartList = [(ytResults[score[0]], score[1]) for score in bestToWorst]
    pp.pprint(smartList[:7])


def parsePandoraLikes(pandoraLikesPage):
    pFile = urllib.request.urlopen(pandoraLikesPage).read()
...
def getTopN(artist, n):
    try:
        ...
        print("SEARCH ERROR: Couldnt find top " + str(
            n) + " songs by " + artist + ". Check that the artist is spelled correctly.")
        return False
...
def getAlbum(artist, album):
    try:
        ...
        soup = BeautifulSoup(r.content, 'html.parser')
        tds = soup.findAll("td", {"class": "chartlist-name"})
        return list(map((lambda td: td.find("a").text), tds))
    except Exception as e:
        print(
            "SEARCH ERROR: Couldnt find album " + album + " by " + artist + ". Check that the album name is spelled correctly.")
        return False


def getSpotifyPlaylists(username, reqPlaylists):
    scope = "playlist-read-private user-library-read"
    token = spotipy.util.prompt_for_user_token(username, scope, SPOTIPY_CLIENT_ID, SPOTIPY_CLIENT_SECRET,
        ...
        for playlist in spPlaylists['items']:
            if (len(reqPlaylists) == 0 or playlist['name'].lower() in [pl.lower() for pl in reqPlaylists]):
                results = sp.user_playlist(user, playlist['id'],
                                           fields="tracks,next")
                songs = list(map((lambda item: item['track']['name']), results['tracks']['items']))
                artists = list(map((lambda item: item['track']['artists'][0]['name']), results['tracks']['items']))
                tracks = list(zip(artists, songs))
...
def compare(file1, file2):
    ...
    misMatches = list(filter((lambda l1_l2: l1_l2[0] != l1_l2[1]), list(zip(c1, c2))))
